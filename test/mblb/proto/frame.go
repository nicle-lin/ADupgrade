...
func ReadFrame(b []byte,conn net.Conn) (n int, err error) {
	...
	if frameFlag != FRAMEFLAG {
		return FRAME_HEADER_LEN,fmt.Errorf("[ReadPacket]frame flag is wrong:0x%x", frameFlag)
	}

	if frameLength > MAX_FRAME_LEN {
		return FRAME_HEADER_LEN,fmt.Errorf("[ReadPacket]SecDataLen wrong:0x%x", frameLength)
	}
